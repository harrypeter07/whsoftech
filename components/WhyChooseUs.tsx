...
const REASONS = [
  ...
  { icon: MessageSquare, title: 'Transparent Communication',     desc: 'Daily updates, weekly demos, and an open WhatsApp channel. You are always in the loop, never waiting for answers.',                         color: '#db2777', stat: '2hr',   statLabel: 'response time' },
  ...
  { icon: Brain,         title: 'AI & Modern Tech Expertise',    desc: 'Our team stays ahead of the curve. We bring cutting-edge AI, automation, and modern frameworks to every project.',                           color: '#f472b6', stat: 'GPT-4', statLabel: 'AI powered' },
...
function StatCounter({ stat, color }: { stat: string; color: string }) {
  ...
  return <div ref={ref} style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: '1.5rem', fontWeight: 800, color, lineHeight: 1 }}>{displayed}</div>;
}

export function WhyChooseUs() {
  return (
    <section id="why-us" style={{ background: '#060f1e', padding: '6rem 0', borderTop: '1px solid rgba(219,39,119,0.08)' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1.5rem' }}>
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} style={{ textAlign: 'center', marginBottom: '3.5rem' }}>
          ...
          <h2 style={{ fontSize: 'clamp(1.75rem, 3.5vw, 2.75rem)', fontWeight: 800, color: '#e2e8ff', marginBottom: '1rem', letterSpacing: '-0.02em' }}>
            The Agency That <span className="grad">Actually Delivers</span>
          </h2>
        ...
        <div className="why-bento">
          {REASONS.map((r, i) => (
            <motion.div key={r.title} initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.08 }}
              style={{ background: '#0f2341', border: '1px solid rgba(59,130,246,0.12)', borderRadius: '18px', padding: '2rem', position: 'relative', overflow: 'hidden', transition: 'border-color 0.25s, box-shadow 0.3s', cursor: 'default' }}
              whileHover={{ y: -4 }}
              onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.borderColor = 'rgba(219,39,119,0.35)'; el.style.boxShadow = '0 16px 40px rgba(219,39,119,0.10)'; }}
              onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.borderColor = 'rgba(59,130,246,0.12)'; el.style.boxShadow = 'none'; }}>
              <div style={{ position: 'absolute', top: '-10px', right: '1rem', fontFamily: 'Space Grotesk, sans-serif', fontSize: '5rem', fontWeight: 900, color: 'rgba(219,39,119,0.04)', lineHeight: 1, pointerEvents: 'none', userSelect: 'none' }}>
                {String(i + 1).padStart(2, '0')}
              </div>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '1.25rem' }}>
                <div style={{ width: '48px', height: '48px', borderRadius: '12px', background: `${r.color}12`, border: `1px solid ${r.color}25`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <r.icon size={22} color={r.color} />
                </div>
                <div style={{ textAlign: 'right' }}>
                  <StatCounter stat={r.stat} color={r.color} />
                  <div style={{ fontSize: '0.65rem', color: '#64748b', fontWeight: 500 }}>{r.statLabel}</div>
                </div>
              </div>
              <h3 style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: '1rem', fontWeight: 700, color: '#e2e8ff', marginBottom: '0.625rem' }}>{r.title}</h3>
              <p style={{ color: '#94a3b8', fontSize: '0.875rem', lineHeight: 1.7 }}>{r.desc}</p>
            </motion.div>
          ))}
        </div>
      </div>
