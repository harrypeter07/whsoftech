...
const plans = [
  {
    ...
    price: "Custom",
    period: " quote",
    features: ["Discovery call", "Project proposal", "Up to 3 revisions"],
    cta: "Get started",
    href: "/contact",
    featured: false,
  },
      ...
      <div className="section-shell">
        ...
        <div className="mx-auto grid max-w-4xl grid-cols-1 gap-6 md:grid-cols-2 md:gap-8">
          {plans.map((plan) => (
            <div
              key={plan.name}
              className={`rounded-2xl border-2 p-6 transition-[border-color,box-shadow] duration-300 md:p-8 ${
                plan.featured
            ...
            >
              <h3
                className={`mb-2 text-xl font-bold ${plan.featured ? "text-sky-300" : "text-white"}`}
              >
                {plan.name}
              </h3>
