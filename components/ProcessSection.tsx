...
export function ProcessSection() {
  return (
    <section id="process" style={{ background: '#030311', padding: '6rem 0', borderTop: '1px solid rgba(59,130,246,0.06)' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1.5rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1.25rem' }}>
            {STEPS.slice(0, 4).map((step, i) => (
              <motion.div key={step.num}
                ...
                style={{ background: '#0e0e2a', border: '1px solid rgba(59,130,246,0.1)', borderRadius: '16px', padding: '1.5rem', position: 'relative', overflow: 'hidden', cursor: 'default', transition: 'border-color 0.25s, box-shadow 0.25s' }}
                ...
                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = 'rgba(59,130,246,0.1)'; (e.currentTarget as HTMLElement).style.boxShadow = 'none'; }}
              >
                {/* Step number watermark */}
                <div style={{ position: 'absolute', top: '-8px', right: '0.75rem', fontFamily: 'Space Grotesk, sans-serif', fontSize: '4rem', fontWeight: 900, color: `${step.color}08`, lineHeight: 1, pointerEvents: 'none' }}>{step.num}</div>
                {/* Arrow connector (not on last in row) */}
                {i < 3 && (
                  <div style={{ position: 'absolute', top: '50%', right: '-10px', transform: 'translateY(-50%)', width: '20px', height: '2px', background: `${step.color}30`, zIndex: 10 }} />
                )}
                ...
                <h3 style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: '0.9375rem', fontWeight: 700, color: '#e2e8ff', marginBottom: '0.5rem' }}>{step.title}</h3>
                <p style={{ color: '#7b8db0', fontSize: '0.8rem', lineHeight: 1.65 }}>{step.desc}</p>
              </motion.div>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'center' }}>
            <svg width="40" height="24" viewBox="0 0 40 24">
              <path d="M20 0 L20 24" stroke="rgba(59,130,246,0.25)" strokeWidth="2" strokeDasharray="4 3" />
              <polygon points="20,24 15,16 25,16" fill="rgba(59,130,246,0.3)" />
            </svg>
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} style={{ textAlign: 'center', marginTop: '3.5rem' }}>
          <p style={{ color: '#7b8db0', marginBottom: '1.5rem', fontSize: '1rem' }}>Ready to start? Let&apos;s kick off with a free consultation.</p>
