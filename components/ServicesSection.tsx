...
const ICON_MAP: Record<string, React.ElementType> = {
  Globe, Layers, Smartphone, Brain, Zap, Palette, ShoppingCart, Search,
};
...
function TiltCard({ children, color, featured }: { children: React.ReactNode; color: string; featured?: boolean }) {
  ...
  const onMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    el.style.boxShadow = featured ? `0 24px 60px ${color}20` : `0 12px 40px rgba(37,99,235,0.10)`;
  };
  const onLeave = () => {
    const el = ref.current; if (!el) return;
...
export function ServicesSection() {
  const [services, setServices] = useState<ServiceItem[]>(SERVICES_STATIC);
          ...
          {services.map((s, i) => {
            ...
            return (
              <motion.div key={s.title}
                initial={s.featured ? { opacity: 0, y: 30, scale: 0.95 } : { opacity: 0, x: i % 2 === 0 ? -60 : 60 }}
                whileInView={s.featured ? { opacity: 1, y: 0, scale: 1 } : { opacity: 1, x: 0 }}
                viewport={{ once: true }}
                transition={{ duration: 0.5, delay: i * 0.06 }}
              >
                <TiltCard color={s.color} featured={s.featured}>
                  {s.featured ? (
                    /* Featured large card — keep dark blue gradient, it pops on the light page */
