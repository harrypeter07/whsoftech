...
function FAQItem({ faq, isOpen, onToggle, color = '#db2777' }: { faq: { q: string; a: string }; isOpen: boolean; onToggle: () => void; color?: string }) {
  return (
    <div style={{ background: '#0f2341', border: `1px solid ${isOpen ? color + '50' : 'rgba(59,130,246,0.12)'}`, borderRadius: '14px', overflow: 'hidden', transition: 'border-color 0.3s' }}>
      <button onClick={onToggle} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', padding: '1.25rem 1.5rem', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left' }}>
        ...
        <div style={{ width: '30px', height: '30px', borderRadius: '50%', background: isOpen ? `${color}15` : 'rgba(59,130,246,0.08)', border: `1px solid ${isOpen ? color + '35' : 'rgba(59,130,246,0.18)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 0.3s' }}>
          {isOpen ? <Minus size={14} color={color} /> : <Plus size={14} color="#94a3b8" />}
        </div>
      </button>
      <AnimatePresence>
        {isOpen && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
            transition={{ type: 'spring', stiffness: 320, damping: 22 }} style={{ overflow: 'hidden' }}>
            <div style={{ padding: '0 1.5rem 1.25rem', color: '#94a3b8', fontSize: '0.9rem', lineHeight: 1.75, borderTop: `1px solid rgba(59,130,246,0.10)`, paddingTop: '1rem' }}>{faq.a}</div>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
...
export function FAQSection() {
  ...
  return (
    <section id="faq" style={{ background: '#060f1e', padding: '6rem 0', borderTop: '1px solid rgba(219,39,119,0.06)' }}>
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 1.5rem' }}>
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} style={{ textAlign: 'center', marginBottom: '3.5rem' }}>
          ...
          <h2 style={{ fontSize: 'clamp(1.75rem, 3.5vw, 2.75rem)', fontWeight: 800, color: '#e2e8ff', marginBottom: '1rem', letterSpacing: '-0.02em' }}>
            Common Questions, <span className="grad">Straight Answers</span>
          </h2>
          <p style={{ color: '#94a3b8', fontSize: '1.05rem' }}>Everything you need to know before starting your project.</p>
        </motion.div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {left.map((faq, i) => (
              <motion.div key={i} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.05 }}>
                <FAQItem faq={faq} isOpen={open === i} onToggle={() => setOpen(open === i ? null : i)} color="#db2777" />
              </motion.div>
            ))}
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {right.map((faq, i) => (
              <motion.div key={i + half} initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.05 }}>
                <FAQItem faq={faq} isOpen={open === i + half} onToggle={() => setOpen(open === i + half ? null : i + half)} color="#3b82f6" />
              </motion.div>
            ))}
          </div>
        </div>
