...
export function FloatingCallButton() {
  ...
  const cardStyle = (borderColor: string): React.CSSProperties => ({
    ...
    background: 'rgba(7,7,30,0.96)', border: `1px solid ${borderColor}`,
    color: 'white', fontSize: '0.85rem', fontWeight: 600,
    textDecoration: 'none', backdropFilter: 'blur(16px)',
    whiteSpace: 'nowrap' as const, boxShadow: '0 8px 30px rgba(0,0,0,0.5)',
  });
  ...
  return (
    <div style={{ position: 'fixed', bottom: '1.75rem', left: '1.75rem', zIndex: 999, display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: '0.625rem' }}>
        ...
        {open && (
          <>
            <motion.a href="mailto:hello@whssofttech.com"
              ...
              transition={{ delay: 0.05, duration: 0.25 }} style={cardStyle('rgba(59,130,246,0.3)')}>
            ...
            <motion.a href="tel:+919876543210"
              initial={{ opacity: 0, x: -20, scale: 0.8 }} animate={{ opacity: 1, x: 0, scale: 1 }} exit={{ opacity: 0, x: -20, scale: 0.8 }}
              transition={{ delay: 0.15, duration: 0.25 }} style={cardStyle('rgba(59,130,246,0.3)')}>
      ...
      <motion.button onClick={() => setOpen(!open)} animate={{ rotate: open ? 45 : 0 }} transition={{ duration: 0.3 }} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}
        style={{ width: '52px', height: '52px', borderRadius: '50%', background: open ? 'rgba(59,130,246,0.12)' : 'linear-gradient(135deg, #2563eb, #1d4ed8)', border: open ? '1px solid rgba(59,130,246,0.3)' : 'none', boxShadow: open ? 'none' : '0 4px 20px rgba(37,99,235,0.5)', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
      ...
      {!open && (
        <motion.div animate={{ scale: [1, 1.7, 1], opacity: [0.5, 0, 0.5] }} transition={{ duration: 2.5, repeat: Infinity, ease: 'easeOut' }}
          style={{ position: 'absolute', bottom: 0, left: 0, width: '52px', height: '52px', borderRadius: '50%', border: '2px solid rgba(59,130,246,0.5)', pointerEvents: 'none' }} />
      )}
    </div>
  );
}
